...
for GAMMA in 1 2 10
	...
	do
		python $ATTACK_PY $DATASET --use_cpu=0 --target_test_train_ratio=$GAMMA --target_model='nn' --target_epochs=100 --target_learning_rate=0.005 --target_privacy='grad_pert' --target_dp='rdp' --target_epsilon=$EPSILON --run=$RUN
	done
	for EPSILON in 0.1 0.5 1.0 5.0 10.0 50.0 100.0
	do
		python $ATTACK_PY $DATASET --use_cpu=0 --target_test_train_ratio=$GAMMA --target_model='nn' --target_epochs=100 --target_learning_rate=0.005 --target_privacy='grad_pert' --target_dp='gdp' --target_epsilon=$EPSILON --run=$RUN
	done
done
echo done
