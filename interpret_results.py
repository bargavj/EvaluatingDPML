...
DP = ['dp_', 'adv_cmp_', 'rdp_', 'zcdp_']
TYPE = ['o-', '.-', '^-', '--']
DP_LABELS = ['Naive Composition', 'Advanced Composition', 'RDP', 'zCDP']
RUNS = range(5)
...
def plot_advantage(result):
	...
	for dp in DP:
		...
		#plt.errorbar(EPSILONS, (baseline_acc - test_acc_mean) / baseline_acc, yerr=test_acc_std, fmt=TYPE[DP.index(dp)], capsize=2, label=DP_LABELS[DP.index(dp)])
		plt.errorbar(EPSILONS, attack_adv_mean, yerr=attack_adv_std, fmt=TYPE[DP.index(dp)], capsize=2, label=DP_LABELS[DP.index(dp)])
	plt.xscale('log')
