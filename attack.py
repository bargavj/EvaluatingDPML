...
def train_target_model(dataset, hold_out_train_data=None, epochs=100, batch_size=100, learning_rate=0.01, l2_ratio=1e-7,
    ...
    output_layer, _, _, train_loss, train_acc, test_acc = train_model(dataset, hold_out_train_data, n_hidden=n_hidden, epochs=epochs, learning_rate=learning_rate,
    ...
    return attack_x, attack_y, classes, train_loss, output_layer, train_acc, test_acc
...
def attribute_inference(true_x, true_y, batch_size, output_layer, train_loss, features):
	...
	for feature in features:
		...
		for batch in iterate_minibatches(low_data, true_y, batch_size, False):
			low_op.append(prob_fn(batch[0]))
		for batch in iterate_minibatches(high_data, true_y, batch_size, False):
			high_op.append(prob_fn(batch[0]))
	    	
		low_op = np.vstack(low_op)
		high_op = np.vstack(high_op)
...
def run_experiment():
	...
	batch_size = args.target_batch_size
	pred_y, membership, test_classes, train_loss, output_layer, train_acc, test_acc = train_target_model(
	    dataset=dataset,
	...
	attr_adv, attr_mem, attr_pred = attribute_inference(true_x, true_y, batch_size, output_layer, train_loss, features)
