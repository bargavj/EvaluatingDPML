from ..code.attack import save_data, load_data, train_target_model, yeom_membership_inference, shokri_membership_inference, yeom_attribute_inference
from ..code.utilities import log_loss, get_random_features
import numpy as np
import argparse
import os
import pickle
...
def run_experiment(args):
    ...
    if args.target_privacy == 'no_privacy':
        pickle.dump([train_acc, test_acc, train_loss, membership, shokri_mem_adv, shokri_mem_confidence, yeom_mem_adv, per_instance_loss, yeom_attr_adv, yeom_attr_mem, yeom_attr_pred, features], open(RESULT_PATH+args.train_dataset+'/'+args.target_model+'_'+'no_privacy_'+str(args.l2_ratio)+'.p', 'rb'))
    else:
